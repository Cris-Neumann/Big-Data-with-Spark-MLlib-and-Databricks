...
En el siguiente proyecto tiene como objetivo predecir la probabilidad de que un cliente no pague el saldo de su tarjeta de crédito en el futuro en función de su perfil de cliente mensual, para lo cual se deberán procesar archivos con millones de registros de transacciones bancarias de American Express, obtenidos desde Kaggle, y especialmente preparado para este fin (https://acortar.link/LOfyfq). Para tener la capacidad de cómputo para estas cantidades de datos, se utilizará la plataforma Databricks que facilita el uso de Apache Spark para estos fines. Para cumplir el objetivo, se usará un algortimo de Machine Learning llamado Gradient Boosting Trees (GBTClassifier) para predecir el incumplimiento crediticio. Para la obtención de los datos, se utilizará la API de Kaggle y un bucket de Amazon S3.
...
* Como paso previo, se debe crear un cluster, el cual para la versión Comunnity Edition, que es gratuita, posee 15 GB de memoria y 2 cores. Adjunto una imagen donde se muestra en qué lugar debe crear el custer, en sección "compute":
<br/><br/>
